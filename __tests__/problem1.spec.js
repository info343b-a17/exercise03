...
describe('HTML supports responsive design', () => {
  ...
  test('HTML includes viewport meta', () => {
    ...
    let contentAttr = meta.attr('content').split(',').sort();
...
describe('Produces the expected visual output', () => {
  ...
  test('on extra-small width screens', async () => {
    ...
    //navigation
    let capture = await page.screenshot({clip:{x:0,y:0,width:480,height:125}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //content
    capture = await page.screenshot({clip:{x:0,y:200,width:480,height:300}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //footer (x: -150, y:-50)
    capture = await page.screenshot({clip:{x:480-150,y:800-50,width:150,height:50}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});
  }, 10000)
  ...
  test('on small width screens', async () => {
    ...
    //navigation
    let capture = await page.screenshot({clip:{x:0,y:0,width:250,height:75}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});
  }, 10000)
  ...
  test('on medium width screens', async () => {
    ...
    //navigation
    let capture = await page.screenshot({clip:{x:0,y:0,width:300,height:75}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //content
    capture = await page.screenshot({clip:{x:100,y:150,width:600,height:200}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //footer (x: -150, y:-50)
    capture = await page.screenshot({clip:{x:800-150,y:480-50,width:150,height:50}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});    
  }, 10000)
  ...
  test('on large width screens', async () => {
    ...
    //navigation
    let capture = await page.screenshot({clip:{x:0,y:0,width:150,height:75}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //content
    capture = await page.screenshot({clip:{x:125,y:200,width:800,height:275}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});

    //footer (x: -150, y:-50)
    capture = await page.screenshot({clip:{x:1024-150,y:768-50,width:150,height:50}});
    expect(capture).toMatchImageSnapshot({customDiffConfig:{
      threshold:0.001 //very small threshold
    }});    
  }, 10000)
